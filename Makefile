...
LIBCOMPONENTS = $(BUILD_OBJ)/bzero.o
...
$(BUILD_OBJ)/timestamp:
	mkdir -p $(BUILD_OBJ) $(BUILD_C) $(BUILD_PY) \
		$(BUILD_LIB) $(BUILD_INC) $(BUILD_IBIN) $(BUILD_H) $(BUILD_INC)/ristretto255 \
		$(PER_OBJ_DIRS)
	touch $@
...
LIBCOMPONENTS += $$(BUILD_OBJ)/$(1)/ristretto.o \
                 $$(BUILD_OBJ)/$(1)/elligator.o \
                 $$(BUILD_OBJ)/$(1)/scalar.o \
                 $$(BUILD_OBJ)/$(1)/ristretto_tables.o

PER_OBJ_DIRS += $$(BUILD_OBJ)/$(1)
GLOBAL_HEADERS_OF_$(1) = $(BUILD_INC)/ristretto$(3)/point.h
...
GEN_CODE_P2 += $(BUILD_C)/$(1)/ristretto_tables.c
GEN_CODE += $$(GEN_CODE_FOR_$(1))
...
$$(BUILD_INC)/ristretto$(3)/point.%: src/per_curve/point.tmpl.% src/generator/* Makefile
	$(PYTHON) -B src/generator/template.py --per=curve --item=$(1) --guard=$$(@:$(BUILD_INC)/%=%) -o $$@ $$<
	
$$(BUILD_INC)/ristretto$(3)/elligator_$(3).%: src/per_curve/elligator.tmpl.% src/generator/* Makefile
	$(PYTHON) -B src/generator/template.py --per=curve --item=$(1) --guard=$$(@:$(BUILD_INC)/%=%) -o $$@ $$<
	
$$(BUILD_INC)/ristretto$(3)/scalar_$(3).%: src/per_curve/scalar.tmpl.% src/generator/* Makefile
	$(PYTHON) -B src/generator/template.py --per=curve --item=$(1) --guard=$$(@:$(BUILD_INC)/%=%) -o $$@ $$<

$$(BUILD_IBIN)/ristretto$(3)_gen_tables: $$(BUILD_OBJ)/$(1)/ristretto_gen_tables.o \
		$$(BUILD_OBJ)/$(1)/ristretto.o $$(BUILD_OBJ)/$(1)/scalar.o $$(BUILD_OBJ)/bzero.o \
		$$(COMPONENTS_OF_$(2))
	$$(LD) $$(LDFLAGS) -o $$@ $$^

$$(BUILD_C)/$(1)/ristretto_tables.c: $$(BUILD_IBIN)/ristretto$(3)_gen_tables
	./$$< > $$@ || (rm $$@; exit 1)
...
$$(BUILD_OBJ)/ristretto$(3)_gen_tables.o: src/ristretto_gen_tables.c $$(HEADERS_OF_$(1))
	$$(CC) $$(CFLAGS) \
		-I build/obj/curve_$(1) -I src/$(2) -I src/$(2)/$$(ARCH_FOR_$(2)) -I src/include/$$(ARCH_FOR_$(2)) \
		-I $(BUILD_H)/$(1) -I $(BUILD_H)/$(2) -I $(BUILD_H)/$(2)/$$(ARCH_FOR_$(2)) \
		-c -o $$@ $$<
